...
public class OrangeUniformStudents extends Thread  {
	...
	public OrangeUniformStudents(int id) {
		// TODO Auto-generated constructor stub
		this.id = id;
		this.setName("OrangeUniformStudent_"+this.id);
		hasGroup = false;
		groupNo = 0;
		paradeLine = 0;
		cantAttend = false;
	...
	public void run() {                      
		...
		while(HalloweenParadeAndPuppeShow.paradeIsOpen){											// continue until the parade is Open
			...
			if(canGoToParade){
				HalloweenParadeAndPuppeShow.ous.add(this);												// add itself to the list of orange student
				...
				if(!cantAttend){                                                                      	// if assigned to a group
					...
					while(!canGoToShow){						// if no group members =  no parade, try next parade
						...
						else
						{
							...
							HalloweenParadeAndPuppeShow.ous.add(this);							//add itself to the list of green student
							HalloweenParadeAndPuppeShow.notifyStudentsWhoSeeksGroup();
							lookForGroup();

							if(!cantAttend){													// if assigned to group for parade
								Teacher.shortNap(100);
								msg("Found group "+groupNo);	
								Teacher.shortNap(100);
								waitForOtherGroups();
								msg("Group: "+groupNo+" starting to march parade");
								inTheParade();													// in the parade after waiting
								msg("Parade ended going for snack break");
							}
						}
					}
	...
	private void lookForGroup()
	{
		...
		synchronized(Teacher.meetingLock){
			while(!hasGroup)
			{
				...
				{
					msg("entering meeting critical section to make groups");

					this.groupNo = ++Teacher.SetGroupNo;
					this.hasGroup = true;
					HalloweenParadeAndPuppeShow.groups.add(this);
					for(int i = 0; i<HalloweenParadeAndPuppeShow.ous.size(); i++)
					{
						if(this==HalloweenParadeAndPuppeShow.ous.get(i))			
						{
							HalloweenParadeAndPuppeShow.ous.remove(i);
							msg("deleted myself from list and haven choosen two team member");
							break;
						}
					}
					HalloweenParadeAndPuppeShow.gus.get(0).groupNo =  Teacher.SetGroupNo;
					HalloweenParadeAndPuppeShow.gus.get(0).hasGroup = true;
					HalloweenParadeAndPuppeShow.groups.add(HalloweenParadeAndPuppeShow.gus.remove(0));
					...
					try {
						msg("gonna wait for more green students");
	...
	private void printTotalStudents()
	{
		int size, size2;
		size = HalloweenParadeAndPuppeShow.gus.size();
		msg("total number of Green student looking for group: "+size);
		size2 = HalloweenParadeAndPuppeShow.ous.size();
		msg("total number of Orange student looking for group: "+size2);
	}
	...
	private void waitForShowToStart() {									// inside the tent wait for the show to start
		...
		synchronized (StaffMember.watchingPuppetShow) {
			...
			if(doneWatching)		// if interrupted, showAttned wont count
			{               
	...
	private void waitForNextShow()										// waiting Students at the puppetShow, waiting for the next show
	{
		...
		else if(cantWaitAfterLastPuppetShow)									// if last puppet show
		{
